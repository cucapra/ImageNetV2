...
#term = 'rate'#rate
    ...
    if 'bayesian' in group:
        df = pd.read_csv('csv/bayesian.csv')
        return (df['rate'][index][20:], df['acc1'][index][20:])
...
groups = {  
            ...
            'sorted1000': {'index': slice(None,1000)},
            'bayesian_prior': { 'index': slice(None) },
            #'sorted1000': { 'index': sorted_index }
         }

...
def train_model(args, model, dataloaders, criterion, optimizer, is_inception=False):
    ...
    for epoch in range(args.num_epochs):
        ...
        for phase in phases:
            ...
            epoch_acc5 = running_corrects_5 / num_images

            print('[{}] Loss:{:.4f}; Acc1:{:.4f}%; Acc5:{:.4f}%; Imgs:{}'.format(
                phase, epoch_loss, epoch_acc*100., epoch_acc5*100., num_images
            ))
            # deep copy the model
            if phase == 'val' and epoch_acc > best_acc:
                ...
                best_model_wts = copy.deepcopy(model.state_dict())
            if phase == 'val':
                val_acc_history.append((epoch_acc,epoch_acc5))
